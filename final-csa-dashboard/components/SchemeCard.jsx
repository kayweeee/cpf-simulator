    ...
    <div className="flex flex-col p-4 border-4 rounded-xl max-w-[350px]">
      ...
      {scheme_button ? (
        <div className="flex flex-row gap-2 pt-6">
          <button onClick={enabled ? onClick : null} className={`${enabled ? 'bg-dark-green text-white py-2 px-4 rounded-md hover:bg-darker-green' : 'bg-gray-400 text-gray-700 py-2 px-4 rounded-md cursor-not-allowed'}`} disabled={!enabled}>
            Start Course
          </button>
        </div>
      ) : null}
